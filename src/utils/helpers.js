 ...
 */
export const isSchemaMulti = (type) => {
 ...
 */
export const isSchemaReference = (type) => {
 ...
 */
export const filterUniqueTags = (tags = []) => {
    return tags.flat(Infinity).filter((firstTag, index) => {
        ...
        return (index ===
            tags.flat(Infinity).findIndex((secondTag) => {
                return (JSON.stringify({ label: secondTag.label, value: secondTag.value }) === firstTagStringified);
            }));
    });
};
/**
 ...
 */
export const setAtPath = (object, path, value) => {
    let props = false;
    if (Array.isArray(path))
        props = path.slice(0);
    if (typeof path === 'string')
