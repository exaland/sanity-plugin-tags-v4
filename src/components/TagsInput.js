...
import { getPredefinedTags, getSelectedTags, getTagsFromReference, getTagsFromRelated, } from '../utils/observables';
...
export const TagsInput = forwardRef((props, ref) => {
    ...
    const { schemaType, // Schema information
    ...
    const { predefinedTags = undefined, includeFromReference = false, includeFromRelated = false, customLabel = 'label', customValue = 'value', allowCreate = true, onCreate = async (val) => {
        const tag = {};
        setAtPath(tag, customLabel, val);
        setAtPath(tag, customValue, val);
        return tag;
    }, checkValid = (inputValue, currentValues) => !currentValues.includes(inputValue) && !!inputValue && inputValue.trim() === inputValue, reactSelectOptions = {}, } = schemaType.options ? schemaType.options : {};
    const resolvedPredefinedTags = Array.isArray(predefinedTags) && !predefinedTags.length ? undefined : predefinedTags;
    ...
    const isReferencePredefinedWarning = schemaType.options && !!resolvedPredefinedTags && isReference;
    ...
    useEffect(() => {
        ...
        let selectedSubscription = defaultSubscription;
        let predefinedSubscription = defaultSubscription;
        let relatedSubscription = defaultSubscription;
        let referenceSubscription = defaultSubscription;
    ...
    const handleCreate = React.useCallback(async (inputValue) => {
        ...
        if (Array.isArray(selected)) {
            handleChange([...selected, newCreateValue]);
        }
        else {
    ...
    const selectOptions = {
        ...
        options,
        value: selected,
        isValidNewOption: (inputValue, selectedValues, selectedOptions) => {
    ...
    return (<>
        ...
        {allowCreate && !isReference ? (<CreatableSelect {...selectOptions}/>) : (<Select {...selectOptions}/>)}
      </>);
});
